...
const Notification: FC<cardProps> = ({ closeModal, notifications }) => {
  ...
  const handleClick = (notification: any) =>{
    ...
    readNotification(userData._id,notification._id).then(data=>{
      router.push(`/trip?tripId=${notification.sender_id}`);
    }).catch(err=>console.log(err));
  }
  const dayDifference = (date: any)=>{
    const sentDate = date.split('T')[0];
    const date1= new Date();
    const diff = date1.getTime()-(new Date(sentDate)).getTime();
    return Math.floor(diff/(1000*60*60*24));
  }
  return (
    <>
      <div
        className="2xl:w-4/12  h-screen overflow-y-auto p-8 absolute right-0 "
        style={{ height: "50vh", top: "60px", zIndex:5,backgroundColor:'#e5e5e5' }}
      >
        ...
        {notifications?.map((notification:any, index: number)=>(<div key={index} className="w-full p-3 mt-4 bg-white rounded flex cursor-pointer" >
          <div
          ...
          <div className="pl-3" onClick={()=>handleClick(notification)}>
            <p className="focus:outline-none text-sm leading-none">
              <span className="text-indigo-700">{notification.title}</span> {notification.decription}
            ...
            <p className="focus:outline-none text-xs leading-3 pt-1 text-gray-500">
              {dayDifference(notification.created_at)} days ago
            </p>
          </div>
        </div>))}
        {/* <div className="w-full p-3 mt-4 bg-red-100 rounded flex items-center">
                          ...
                          <div className="pl-3 w-full flex items-center justify-between">
                              <p  className="focus:outline-none text-sm leading-none text-red-700">Low on storage: 2.5/32gb remaining</p>
        ...
        <div className="flex items-center justiyf-between">
          ...
          <p className="focus:outline-none text-sm flex flex-shrink-0 leading-normal px-3 py-16 text-gray-500">
            {(notifications)?'Thats it for now':'No notifications found!'}
          </p>
          <hr className="w-full" />
        </div>
      </div>
